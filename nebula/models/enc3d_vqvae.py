...
Implements the VQ-VAE encoder class used to encode 3D voxels to latent embeddings
...
class VectorQuantizer(nn.Module):
    ...
    def forward(self, inputs):        
        ...
        return loss, quantized.permute(0, 4, 1, 2, 3).contiguous(), perplexity, encodings, self._embedding.weight


class Encoder3D_VQVAE(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        loss, quantized, perplexity, _, codebook = self._vq_vae(x) 

        return quantized, x, codebook 